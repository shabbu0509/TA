...
export default class Middleware {
    ...
    static fetchOSHistoricalTrends(market) {
        ...
        return dispatch => {
            ...
            // httpCall("splunk", "GET", baseURL + `get-trend?market=${market.name}&filter_level=osh&earliest=${earliest}&latest=${latest}&timespan=${timespan}`)
            //     .then(response =>{
            //         if(response.error){
            //             dispatch(Actions.setServerError(response));
            //         } else {
            ...
            //                     kpiName.push(v.kpi.replace('Placed Orders', 'Order Volume'))
            ...
            //                 trendData.push(addtoTrendData);
                           
            //             dispatch(Actions.setOSHistoricalTrends(trendData))
            //             dispatch(Actions.setIsLoading(false))
